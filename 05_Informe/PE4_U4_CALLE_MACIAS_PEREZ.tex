...
%======================================================
\section{Anexos D, E y F}
%======================================================

\subsection*{Anexo D -- Exportación CSV del backlog}
\textbf{Pendiente.} El archivo \texttt{backlog\_export.csv} se generará desde la herramienta CASE (Jira o Trello) una vez ejecutado el Paso~3 sobre una cuenta real del equipo (Sección~\ref{sec:evidenciacion-case}). No se incluye en esta entrega un CSV sintético, conforme al criterio de autenticidad A2 de la rúbrica; \texttt{04\_Trazabilidad/README.md} detalla el procedimiento exacto para generarlo.

\subsection*{Anexo E -- Capturas del tablero CASE y del repositorio Git}
\textbf{Pendiente.} Las capturas del tablero CASE (tablero completo, detalle de $\geq$3 issues, panel de estadísticas) y del repositorio Git (\texttt{git log --oneline --graph --decorate}, \texttt{git tag -n}) se incorporarán una vez ejecutados los Pasos~3 y~4 sobre las cuentas reales del equipo. Las carpetas \texttt{04\_Trazabilidad/capturas/} y \texttt{06\_Evidencias/capturas\_git/} contienen sendos \texttt{README.md} con los pasos exactos pendientes, conforme al criterio de autenticidad A2 de la rúbrica.

\subsection*{Anexo F -- Referencias IEEE y declaración de uso de IA}
Conforme a la regla de citación de la Sección~3 de la Rúbrica PE4, la siguiente tabla declara la herramienta de inteligencia artificial utilizada durante la elaboración de este informe, la tarea específica en la que se empleó y la verificación crítica realizada por el equipo. El detalle completo de esta declaración, firmado por los tres integrantes, se conserva en \texttt{06\_Evidencias/declaracion\_IA.pdf}.
