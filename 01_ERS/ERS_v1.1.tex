...
\hline
3.1 \ (ERS\_v1.1, PE4) & \ Semana 14 -- 2026--2027 PPA & Equipo Grupo C & \textbf{Línea base PE4 -- Unidad IV.} Publicada tras la inspección Fagan (03/08/2026) y el Acta del CCB (03/08/2026, \texttt{03\_CCB/Acta\_CCB.pdf}). Incorpora: (1) RFC-01 -- nuevo requisito RF-26 ``Registrar y consultar guía de movilización''; (2) RFC-02 -- columna Fuente/EV añadida a la tabla de requisitos no funcionales (Sección~3.3); (3) RFC-03 -- numeración formal RD-1 a RD-5 en las restricciones de diseño (Sección~3.7); y la corrección del 100\,\% de los defectos Crítico/Mayor del Anexo~B con redacción de corrección única (D-01, D-06, D-07, D-08, D-15, D-17, D-18), más las correcciones Menor D-05 y D-11. Los defectos D-02, D-09 y D-10 (``requiere revisión adicional'') quedan expresamente fuera del alcance de este CCB, según el Acta, y se documentan como pendientes en la Sección~\ref{sec:pendientes-v11}. \\
\hline
\end{longtable}
...
\begin{figure}[H]
    ...
    \caption{MU-09 Interfaz del módulo de cámaras frigoríficas del Sistema}
    \label{fig:diagrama_clases}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{MU-20 Interfaz del módulo de roles del Sistema}
    \label{fig:diagrama_clases}
\end{figure}
...
\rfitem{RF-06}{Registrar despiece y clasificación de cortes}{El sistema deberá permitir registrar los cortes obtenidos al despostar un animal (brazos, piernas, costillas, lomos, cabezas) y su peso individual.}{Propietaria -- Bloque 5, Preguntas 23 y 25}{Must}{Estable}{La suma de los pesos de los cortes no supera el peso del animal ingresado, con un margen de merma configurable.}{Dependencias: RF-03, RF-04. EV-05.}
...
\rfitem{RF-26}{Registrar y consultar guía de movilización}{El sistema deberá permitir registrar y consultar la guía de movilización asociada a un lote o animal, como documento distinto de la guía de origen (RF-02), conforme a la distinción declarada en el glosario (Sección~1.3) y en el alcance (Sección~1.2).}{Glosario (1.3) y Alcance (1.2) del ERS -- incorporado mediante RFC-01, aprobado por el CCB de PE4 el 03/08/2026 (Anexo~C, \texttt{03\_CCB/RFC-01.pdf}; Acta, \texttt{03\_CCB/Acta\_CCB.pdf})}{Should}{Estable}{Toda guía de movilización almacenada queda disponible para consulta por número de arete o código de lote, de forma independiente de la guía de origen.}{Dependencias: RF-02. EV-26. \textbf{[Incorporado en PE4 v1.1 -- Corrección D-19 / RFC-01; ver matriz de trazabilidad, fila 41, Sección~5.2.]}}
...
\textit{Columna ``Fuente/EV'' incorporada en PE4~v1.1 mediante RFC-02 (aprobado con alcance mínimo por el CCB, 03/08/2026 -- Corrección D-14; Anexo~C, \texttt{03\_CCB/RFC-02.pdf}), para igualar el nivel de atributos de la tabla de RF (Sección~3.2) sin ampliar además Prioridad/Estabilidad/Criterio de aceptación en esta iteración, conforme al alcance decidido en el Acta del CCB.}

\begin{longtable}{|p{1.2cm}|p{2.5cm}|p{8.3cm}|p{1.6cm}|}
\hline
\textbf{ID} & \textbf{Característica ISO 25010} & \textbf{Descripción cuantificable} & \textbf{Fuente/EV} \\
\hline
RNF-01 & Eficiencia de desempeño & El cálculo del costo total al registrar un pesaje deberá completarse en menos de 1 segundo bajo condiciones normales de operación. & EV-04 \\
\hline
RNF-02 & Fiabilidad & El sistema deberá estar disponible al menos el 98\% del tiempo mensual, considerando ventanas de mantenimiento fuera del horario de faenamiento (2:00--10:00 a.m.). & EV-28 \\
\hline
RNF-03 & Seguridad & El acceso al sistema deberá requerir autenticación mediante usuario y contraseña, con contraseñas almacenadas usando un algoritmo de hash con salting (por ejemplo, bcrypt). & EV-01 \\
\hline
RNF-04 & Interacción de usuario (Usabilidad) & Un encargado de bodega sin experiencia previa en sistemas informáticos deberá poder completar el registro de un ingreso de lote en un máximo de 5 minutos tras una capacitación de 30 minutos. & EV-09 \\
\hline
...
\hline
RNF-07 & Seguridad (integridad de datos) & Todo registro de baja de producto deberá quedar auditado con usuario, fecha y hora, y no podrá eliminarse una vez aprobado por el administrador general. & EV-07 \\
\hline
RNF-08 & Adecuación funcional (completitud) & El conjunto de RF \textit{Debe tener} implementado en el MVP deberá cubrir al menos el 60\% de los RF \textit{Must} del ERS, verificable mediante la matriz de trazabilidad de la Sección~5.2. & EV-16 \\
\hline
...
\hline
RNF-10 & Flexibilidad (adaptabilidad) & El sistema deberá permitir agregar un nuevo tipo de producto (por ejemplo, un nuevo tipo de embutido) y su límite de vida útil sin requerir despliegue de una nueva versión del software, mediante configuración paramétrica. & EV-11 \\
\hline
RNF-11 & Eficiencia de desempeño & La generación de un reporte gerencial filtrado por un rango de hasta 90 días deberá completarse en menos de 5 segundos con un volumen de hasta 10\,000 registros. & EV-08 \\
\hline
...
\hline
RNF-13 & Interacción de usuario (accesibilidad) & Las pantallas críticas de bodega (ingreso, pesaje) deberán tener un contraste mínimo de 4.5:1 y controles táctiles de al menos 44$\times$44 px, considerando el uso con guantes en la planta. & EV-09 \\
\hline
RNF-14 & Fiabilidad (recuperabilidad) & Ante un corte de energía o pérdida de conectividad, el sistema deberá conservar sin pérdida las operaciones registradas en modo offline (RF-21) y recuperarlas al reiniciar el dispositivo. & EV-13 \\
\hline
RNF-15 & Mantenibilidad (capacidad de prueba) & El código fuente del MVP deberá contar con una cobertura de pruebas automatizadas de al menos el 40\% de las líneas correspondientes a los módulos de pesaje y trazabilidad. & EV-16 \\
\hline
\end{longtable}
...
\textit{Numeración RD-1 a RD-5 incorporada en PE4~v1.1 mediante RFC-03 (aprobado por unanimidad por el CCB, 03/08/2026 -- Corrección D-16; Anexo~C, \texttt{03\_CCB/RFC-03.pdf}), para formalizar identificadores previamente referenciados en el texto (por ejemplo, ``RD-2'' en el punto 5) sin estar declarados en su propia fuente.}
\begin{enumerate}[nosep,label=\textbf{RD-\arabic*.}]
...
\subsection{Diagrama de actividad -- CU-01}
\begin{figure}[H]
    \centering
...
\subsection{Diagrama de actividad -- CU-02}
\begin{figure}[H]
    \centering
...
\subsection{Diagrama de actividad -- CU-03}
\begin{figure}[H]
    \centering
...
\subsection{Diagrama de actividad -- CU-04}
\begin{figure}[H]
    \centering
...
\subsection{Diagrama de actividad -- CU-05}
\begin{figure}[H]
    \centering
...
\subsection{Diagrama de actividad -- CU-06}
\begin{figure}[H]
    \centering
...
\hline
1 & -- & Trazabilidad de origen & Propietaria & EV-01 & RF-01 & --\textsuperscript{d17} & HU-01 & CA-01 & Backend-Proveedores & MU-02 \\
...
14 & -- & Desempeño del negocio & Propietaria & EV-08 & RF-14 & CU-12 & -- & -- & Backend-Reportes & MU-18 \\
...
17 & -- & Georreferencia & Admin. gral. & EV-10 & RF-17 & --\textsuperscript{d17} & -- & -- & Backend-Proveedores & MU-04 \\
18 & -- & Historial de precios & Propietaria & EV-11 & RF-18 & -- & -- & -- & Backend-Precios & MU-14 \\
19 & -- & Configurar precios & Propietaria & EV-27 & RF-19 & CU-03 & HU-19 & CA-19 & Backend-Precios & MU-14 \\
...
27 & -- & Disponibilidad del servicio & Todos & EV-28 & RNF-02 & -- & -- & -- & Infraestructura & -- \\
...
\hline
41 & -- & Movilización de producto & Propietaria & EV-26 & RF-26 & CU-22\textsuperscript{b} & HU-22\textsuperscript{b} & CA-22\textsuperscript{b} & Backend-Guias & -- \\
\hline
\end{longtable}
\normalsize

\footnotesize
\textsuperscript{d17} \textbf{Nota de corrección D-17 (PE4 v1.1).} La inspección Fagan detectó que las filas 1 (RF-01) y 17 (RF-17) de esta matriz mapeaban ambas a CU-01, pese a que la especificación textual de CU-01 (Sección~4.2) describe el flujo de RF-03 (``Registrar ingreso de animal/lote''), no el de RF-01 (``Registrar proveedor'') ni el de RF-17 (``Registrar georreferencia de proveedor/granja''). Conforme al Acta del CCB de PE4, se corrige la matriz retirando la referencia incorrecta a CU-01 en ambas filas (marcadas ``--'') en lugar de mantener un enlace de trazabilidad falso; la redacción de los casos de uso dedicados CU-13 (Registrar proveedor) y CU-14 (Registrar georreferencia) queda planificada como trabajo pendiente para la Entrega 4 (2B), según se documenta en la Sección~\ref{sec:pendientes-v11}.

\textsuperscript{b} \textbf{Nota RFC-01.} Los identificadores CU-22, HU-22 y CA-22 son placeholders acordados en el Acta del CCB (03\_CCB/Acta\_CCB.pdf, acuerdo final~1) para reservar la fila de trazabilidad de RF-26; su especificación detallada (caso de uso, historia de usuario y criterio de aceptación completos) queda planificada para la Entrega 4 (2B), en el mismo tratamiento que la descomposición WSJF de la Sección~5.1.
\normalsize

%======================================================
\section{Pendientes declarados al cierre de PE4 v1.1}
\label{sec:pendientes-v11}
%======================================================
Conforme al Acta del Comité de Control de Cambios de PE4 (\texttt{03\_CCB/Acta\_CCB.pdf}), los siguientes defectos del Anexo~B quedan expresamente fuera del alcance de la línea base ERS~v1.1 y se difieren a una fase posterior de revisión, previa consulta con el docente sobre el alcance exacto exigido por la rúbrica:

\begin{itemize}[nosep]
\item \textbf{D-02} (Mayor) -- RF-15 (Gestionar usuarios y roles) sin caso de uso dedicado en la matriz de trazabilidad (fila 15).
\item \textbf{D-09} (Mayor) -- Mockup de ``Precios'' referenciado en la matriz (filas 18--19, 22) sin catalogar de forma independiente en el repositorio de interfaces.
\item \textbf{D-10} (Mayor) -- Diagramas de actividad existentes solo para CU-01 a CU-06 (Sección~4.5--4.10); pendientes CU-07 a CU-12.
\item \textbf{D-03} (Menor) -- Caso de uso ``Apply promotional discount'' presente en el diagrama general de casos de uso (Figura~\ref{fig:diagrama_clases}) sin RF que lo respalde; su corrección requiere regenerar el diagrama en la herramienta de modelado original, tarea que excede el alcance documental de esta línea base y queda pendiente para la Entrega 4 (2B).
\item \textbf{CU-13, CU-14} -- Casos de uso dedicados para RF-01 y RF-17 (véase la nota D-17 de la Sección~5.2).
\end{itemize}

Estos pendientes no afectan el cumplimiento del gatekeeper de corrección de la Rúbrica PE4 (100\,\% de defectos Crítico y Mayor con redacción de corrección única aplicados en el Anexo~B), dado que D-02, D-09 y D-10 fueron clasificados en el informe PE4 como ``requiere revisión adicional'' y quedaron formalmente diferidos por el CCB, no omitidos.
